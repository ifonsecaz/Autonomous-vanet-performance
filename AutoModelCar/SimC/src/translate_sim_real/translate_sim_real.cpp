...
int pausaVal=0;
void evadeCallback(const std_msgs::Int16& msg)
{ 
	pausaVal=(int)msg.data;
}
...
public:
  ...
  TranslationSimReal(ros::NodeHandle &nh)
  {
    ...
    nh_.param<std::string>(node_name+"/topico_steering_sim_gary", topico_steering_sim_gary, "/a0/manual_control/steering"); //"/AutoNOMOS_mini/manual_control/steering"
    nh_.param<std::string>(node_name+"/topico_velocidad_sim_gary", topico_velocidad_sim_gary, "/a0/manual_control/velocity");

    nh_.param<std::string>(node_name+"/topico_steering_real", topico_steering_real, "/AutoNOMOS_mini/manual_control/steering");
    nh_.param<std::string>(node_name+"/topico_velocidad_real", topico_velocidad_real, "/AutoNOMOS_mini/manual_control/speed");
  ...
  void set_evadeA(int msg){
    pause=msg;
    ROS_INFO("Rec evade [%i]", pause);
 }
  ...
  void get_steering_sim(const std_msgs::Float64& val)
  {
    ...
    //evade = nh_.subscribe("/evadeA", MY_ROS_QUEUE_SIZE, &TranslationSimReal::get_evadeA, this);////
