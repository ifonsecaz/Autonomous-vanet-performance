...
void local_planner::get_evadeA(const std_msgs::Int16& msg){
    pause=(int)msg.data; 
...
void local_planner::ackerman_control(cv::Mat& imagePaint) {
    ...
    if (puntosValidos) {
        ...
        if (!std::isnan(steering)) {
            ...
            // falta PID
            //evade = nh_.subscribe("/evadeA", MY_ROS_QUEUE_SIZE, &local_planner::get_evadeA, this);////
...
void local_planner::plot_polinomials(cv::Mat& image) {
    ...
    for (int i = 0; i < image_height; i++) {
        ...
        cv::line(image, cv::Point(0, i), cv::Point(160, i), cv::Scalar(0, 0, 0), 1, cv::LINE_AA);
    }
