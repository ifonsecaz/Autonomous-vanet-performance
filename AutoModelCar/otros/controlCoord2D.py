...
def movimiento (xgoal, ygoal):
	...
	cmd_vel_topic = '/AutoNOMOS_mini/manual_control/speed'
	steering_message = Int16
	cmd_ster_topic = '/AutoNOMOS_mini/manual_control/steering'
	...
	while(True):
		...
		if linear_speed > 1000:
			...
			if linear_speed < -2000:
				linear_speed = -2000
			else:
				if (linear_speed >-70 and linear_speed <0):
					linear_speed=-70
...
if __name__ == '__main__':
	try:
		...
		position_topic = "/AutoNOMOS_mini/real_pose_from_gazebo"
		pose_subscriber = rospy.Subscriber(position_topic, Pose2D, poseCallback)
